...
def make_flow_data(base_dir, flow_thresh):
	...
	for i, cur_seq in enumerate(seqs_all):
		for fb in ['into_future', 'into_past']:            
			for lr in ['left', 'right']:
				im_paths = sorted(glob.glob(os.path.join(img_dir, cur_seq, lr, '*.png')))
				fl_paths = sorted(glob.glob(os.path.join(flow_dir, cur_seq, fb, lr, '*.pfm')))
				if fb == 'into_past':
					im_paths = im_paths[::-1]
					fl_paths = fl_paths[::-1]
	...
	print('{} left for Monkaa.'.format(len(data[0])/len(raw_data[0])))
...
def make_disp_data(base_dir, disp_thresh):
	...
	seqs_all = os.listdir(img_dir)
	for i, cur_seq in enumerate(seqs_all):
		im_paths = sorted(glob.glob(os.path.join(img_dir, cur_seq, 'left/*.png')))
		dp_paths = sorted(glob.glob(os.path.join(disp_dir, cur_seq, 'left/*.pfm')))
...
if __name__ == '__main__':
	...
	# # with open('Monkaa_flow_train.pkl', 'wb') as f:
	# # 	pickle.dump({'flow_train_data': flow_train_data,
	# # 				 'flow_test_data': flow_test_data},
	# # 				 f, pickle.HIGHEST_PROTOCOL
	# # 				 )
	# with open('Monkaa_flow_train.pkl', 'rb') as f:
	...
	# # with open('Monkaa_disp_train.pkl', 'wb') as f:
	# # 	pickle.dump({'disp_train_data': disp_train_data,
	# # 				 'disp_test_data': disp_test_data},
	# # 				 f, pickle.HIGHEST_PROTOCOL
	# # 				 )
	# with open('Monkaa_disp_train.pkl', 'rb') as f:
