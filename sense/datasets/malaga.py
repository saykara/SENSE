...
def find_closest_time_index(imu_list, time):
    return min(range(len(imu_list)), key=lambda i: abs(imu_list[i][0]-time))
...
def calc_pose_diff(imu_list, cur_time, next_time):
    ...
    for i in range(cur_idx - 1, nxt_idx):
        delta_t = imu_np[i + 1][0] - imu_np[i][0]
        angle +=  imu_np[i + 1][4:7] * delta_t
        speed = imu_np[i + 1][0:3] * delta_t
        position += speed * delta_t
    return np.concatenate((position, angle)).tolist()
    ...
    for seq in sequence_list:
        ...
        for j in range(0, len(image_list) - 12, 2):
            ...
            for k in range(0, 5, 2):
                cur_left = os.path.join(path, seq, seq + "_rectified_1024x768_Images", image_list[j + k])
                cur_right = os.path.join(path, seq, seq + "_rectified_1024x768_Images", image_list[j + k + 1])
                nxt_left = os.path.join(path, seq, seq + "_rectified_1024x768_Images", image_list[j + k + 2])
                nxt_right = os.path.join(path, seq, seq + "_rectified_1024x768_Images", image_list[j + k + 3])
                sequence.append([cur_left, cur_right, nxt_left, nxt_right, calc_pose_diff(pose_list, image_list[j + k].split("_")[2], image_list[j + k + 2].split("_")[2])])
