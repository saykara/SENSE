...
class PWCNextDispDecoder(nn.Module):
    def __init__(self, md=4, maxdisp=192, 
        ...
        od = nd
        self.disp_conv5_0, self.disp_res5_0 = make_dec_layer(od,      encoder_planes[0], kernel_size=kernel_size, stride=1, padding=pad, bn_type=bn_type)
        self.disp_conv5_1, self.disp_res5_1 = make_dec_layer(od+dd[0],encoder_planes[1], kernel_size=kernel_size, stride=1, padding=pad, bn_type=bn_type)
        self.disp_conv5_2, self.disp_res5_2 = make_dec_layer(od+dd[1],encoder_planes[2],  kernel_size=kernel_size, stride=1, padding=pad, bn_type=bn_type)
        self.disp_conv5_3, self.disp_res5_3 = make_dec_layer(od+dd[2],encoder_planes[3],  kernel_size=kernel_size, stride=1, padding=pad, bn_type=bn_type)
        self.disp_conv5_4, self.disp_res5_4 = make_dec_layer(od+dd[3],encoder_planes[4],  kernel_size=kernel_size, stride=1, padding=pad, bn_type=bn_type)
        if do_class:
        ...
        if pred_occ and cat_occ:
            od += 2
        self.disp_conv4_0, self.disp_res4_0 = make_dec_layer(od,      encoder_planes[0], kernel_size=kernel_size, stride=1, padding=pad, bn_type=bn_type)
        self.disp_conv4_1, self.disp_res4_1 = make_dec_layer(od+dd[0],encoder_planes[1], kernel_size=kernel_size, stride=1, padding=pad, bn_type=bn_type)
        self.disp_conv4_2, self.disp_res4_2 = make_dec_layer(od+dd[1],encoder_planes[2], kernel_size=kernel_size, stride=1, padding=pad, bn_type=bn_type)
        self.disp_conv4_3, self.disp_res4_3 = make_dec_layer(od+dd[2],encoder_planes[3], kernel_size=kernel_size, stride=1, padding=pad, bn_type=bn_type)
        self.disp_conv4_4, self.disp_res4_4 = make_dec_layer(od+dd[3],encoder_planes[4], kernel_size=kernel_size, stride=1, padding=pad, bn_type=bn_type)
        if do_class:
        ...
        if pred_occ and cat_occ:
            od += 2
        self.disp_conv3_0, self.disp_res3_0 = make_dec_layer(od,      encoder_planes[0], kernel_size=kernel_size, stride=1, padding=pad, bn_type=bn_type)
        self.disp_conv3_1, self.disp_res3_1 = make_dec_layer(od+dd[0],encoder_planes[1], kernel_size=kernel_size, stride=1, padding=pad, bn_type=bn_type)
        self.disp_conv3_2, self.disp_res3_2 = make_dec_layer(od+dd[1],encoder_planes[2], kernel_size=kernel_size, stride=1, padding=pad, bn_type=bn_type)
        self.disp_conv3_3, self.disp_res3_3 = make_dec_layer(od+dd[2],encoder_planes[3], kernel_size=kernel_size, stride=1, padding=pad, bn_type=bn_type)
        self.disp_conv3_4, self.disp_res3_4 = make_dec_layer(od+dd[3],encoder_planes[4], kernel_size=kernel_size, stride=1, padding=pad, bn_type=bn_type)
        if do_class:
        ...
        if pred_occ and cat_occ:
            od += 2
        self.disp_conv2_0, self.disp_res2_0 = make_dec_layer(od,      encoder_planes[0], kernel_size=kernel_size, stride=1, padding=pad, bn_type=bn_type)
        self.disp_conv2_1, self.disp_res2_1 = make_dec_layer(od+dd[0],encoder_planes[1], kernel_size=kernel_size, stride=1, padding=pad, bn_type=bn_type)
        self.disp_conv2_2, self.disp_res2_2 = make_dec_layer(od+dd[1],encoder_planes[2], kernel_size=kernel_size, stride=1, padding=pad, bn_type=bn_type)
        self.disp_conv2_3, self.disp_res2_3 = make_dec_layer(od+dd[2],encoder_planes[3], kernel_size=kernel_size, stride=1, padding=pad, bn_type=bn_type)
        self.disp_conv2_4, self.disp_res2_4 = make_dec_layer(od+dd[3],encoder_planes[4], kernel_size=kernel_size, stride=1, padding=pad, bn_type=bn_type)
        if do_class:
    ...
    def forward(self, x1, x2):
        ...
        x = torch.cat((disp_corr5, self.disp_conv5_0(disp_corr5) + self.disp_res5_0(disp_corr5)),1)
        x = torch.cat((x, self.disp_conv5_1(x) + self.disp_res5_1(x)),1)
        x = torch.cat((x, self.disp_conv5_2(x) + self.disp_res5_2(x)),1)
        x = torch.cat((x, self.disp_conv5_3(x) + self.disp_res5_3(x)),1)
        x = torch.cat((x, self.disp_conv5_4(x) + self.disp_res5_4(x)),1)
        if self.do_class:
        ...
        x = torch.cat((x, self.disp_conv4_0(x) + self.disp_res4_0(x)),1)
        x = torch.cat((x, self.disp_conv4_1(x) + self.disp_res4_1(x)),1)
        x = torch.cat((x, self.disp_conv4_2(x) + self.disp_res4_2(x)),1)
        x = torch.cat((x, self.disp_conv4_3(x) + self.disp_res4_3(x)),1)
        x = torch.cat((x, self.disp_conv4_4(x) + self.disp_res4_4(x)),1)
        if self.do_class:
        ...
        x = torch.cat((x, self.disp_conv3_0(x) + self.disp_res3_0(x)),1)
        x = torch.cat((x, self.disp_conv3_1(x) + self.disp_res3_1(x)),1)
        x = torch.cat((x, self.disp_conv3_2(x) + self.disp_res3_2(x)),1)
        x = torch.cat((x, self.disp_conv3_3(x) + self.disp_res3_3(x)),1)
        x = torch.cat((x, self.disp_conv3_4(x) + self.disp_res3_4(x)),1)
        if self.do_class:
        ...
        x = torch.cat((x, self.disp_conv2_0(x) + self.disp_res2_0(x)),1)
        x = torch.cat((x, self.disp_conv2_1(x) + self.disp_res2_1(x)),1)
        x = torch.cat((x, self.disp_conv2_2(x) + self.disp_res2_2(x)),1)
        x = torch.cat((x, self.disp_conv2_3(x) + self.disp_res2_3(x)),1)
        x = torch.cat((x, self.disp_conv2_4(x) + self.disp_res2_4(x)),1)
        if self.do_class:
...
class PWCNextFlowDecoder(nn.Module):
    def __init__(self, md=4,
        ...
        od = nd
        self.flow_conv5_0, self.flow_res5_0 = make_dec_layer(od,      encoder_planes[0], kernel_size=kernel_size, stride=1, padding=pad, bn_type=bn_type)
        self.flow_conv5_1, self.flow_res5_1 = make_dec_layer(od+dd[0],encoder_planes[1], kernel_size=kernel_size, stride=1, padding=pad, bn_type=bn_type)
        self.flow_conv5_2, self.flow_res5_2 = make_dec_layer(od+dd[1],encoder_planes[2], kernel_size=kernel_size, stride=1, padding=pad, bn_type=bn_type)
        self.flow_conv5_3, self.flow_res5_3 = make_dec_layer(od+dd[2],encoder_planes[3], kernel_size=kernel_size, stride=1, padding=pad, bn_type=bn_type)
        self.flow_conv5_4, self.flow_res5_4 = make_dec_layer(od+dd[3],encoder_planes[4], kernel_size=kernel_size, stride=1, padding=pad, bn_type=bn_type)
        ...
        if pred_occ and cat_occ:
            od += 2
        self.flow_conv4_0, self.flow_res4_0 = make_dec_layer(od,      encoder_planes[0], kernel_size=kernel_size, stride=1, padding=pad, bn_type=bn_type)
        self.flow_conv4_1, self.flow_res4_1 = make_dec_layer(od+dd[0],encoder_planes[1], kernel_size=kernel_size, stride=1, padding=pad, bn_type=bn_type)
        self.flow_conv4_2, self.flow_res4_2 = make_dec_layer(od+dd[1],encoder_planes[2], kernel_size=kernel_size, stride=1, padding=pad, bn_type=bn_type)
        self.flow_conv4_3, self.flow_res4_3 = make_dec_layer(od+dd[2],encoder_planes[3], kernel_size=kernel_size, stride=1, padding=pad, bn_type=bn_type)
        self.flow_conv4_4, self.flow_res4_4 = make_dec_layer(od+dd[3],encoder_planes[4], kernel_size=kernel_size, stride=1, padding=pad, bn_type=bn_type)
        ...
        if pred_occ and cat_occ:
            od += 2
        self.flow_conv3_0, self.flow_res3_0 = make_dec_layer(od,      encoder_planes[0], kernel_size=kernel_size, stride=1, padding=pad, bn_type=bn_type)
        self.flow_conv3_1, self.flow_res3_1 = make_dec_layer(od+dd[0],encoder_planes[1], kernel_size=kernel_size, stride=1, padding=pad, bn_type=bn_type)
        self.flow_conv3_2, self.flow_res3_2 = make_dec_layer(od+dd[1],encoder_planes[2], kernel_size=kernel_size, stride=1, padding=pad, bn_type=bn_type)
        self.flow_conv3_3, self.flow_res3_3 = make_dec_layer(od+dd[2],encoder_planes[3], kernel_size=kernel_size, stride=1, padding=pad, bn_type=bn_type)
        self.flow_conv3_4, self.flow_res3_4 = make_dec_layer(od+dd[3],encoder_planes[4], kernel_size=kernel_size, stride=1, padding=pad, bn_type=bn_type)
        ...
        if pred_occ and cat_occ:
            od += 2
        self.flow_conv2_0, self.flow_res2_0 = make_dec_layer(od,      encoder_planes[0], kernel_size=kernel_size, stride=1, padding=pad, bn_type=bn_type)
        self.flow_conv2_1, self.flow_res2_1 = make_dec_layer(od+dd[0],encoder_planes[1], kernel_size=kernel_size, stride=1, padding=pad, bn_type=bn_type)
        self.flow_conv2_2, self.flow_res2_2 = make_dec_layer(od+dd[1],encoder_planes[2], kernel_size=kernel_size, stride=1, padding=pad, bn_type=bn_type)
        self.flow_conv2_3, self.flow_res2_3 = make_dec_layer(od+dd[2],encoder_planes[3], kernel_size=kernel_size, stride=1, padding=pad, bn_type=bn_type)
        self.flow_conv2_4, self.flow_res2_4 = make_dec_layer(od+dd[3],encoder_planes[4], kernel_size=kernel_size, stride=1, padding=pad, bn_type=bn_type)
    ...
    def forward(self, x1, x2):
        ...
        x = torch.cat((flow_corr5, self.flow_conv5_0(flow_corr5) + self.flow_res5_0(flow_corr5)),1)
        x = torch.cat((x, self.flow_conv5_1(x) + self.flow_res5_1(x)),1)
        x = torch.cat((x, self.flow_conv5_2(x) + self.flow_res5_2(x)),1)
        x = torch.cat((x, self.flow_conv5_3(x) + self.flow_res5_3(x)),1)
        x = torch.cat((x, self.flow_conv5_4(x) + self.flow_res5_4(x)),1)
        ...
        if self.pred_occ and self.cat_occ:
            x = torch.cat((x, F.interpolate(occ5, scale_factor=2, mode='bilinear')), 1)
        x = torch.cat((x, self.flow_conv4_0(x) + self.flow_res4_0(x)),1)
        x = torch.cat((x, self.flow_conv4_1(x) + self.flow_res4_1(x)),1)
        x = torch.cat((x, self.flow_conv4_2(x) + self.flow_res4_2(x)),1)
        x = torch.cat((x, self.flow_conv4_3(x) + self.flow_res4_3(x)),1)
        x = torch.cat((x, self.flow_conv4_4(x) + self.flow_res4_4(x)),1)
        ...
        if self.pred_occ and self.cat_occ:
            x = torch.cat((x, F.interpolate(occ4, scale_factor=2, mode='bilinear')), 1)
        x = torch.cat((x, self.flow_conv3_0(x) + self.flow_res3_0(x)),1)
        x = torch.cat((x, self.flow_conv3_1(x) + self.flow_res3_1(x)),1)
        x = torch.cat((x, self.flow_conv3_2(x) + self.flow_res3_2(x)),1)
        x = torch.cat((x, self.flow_conv3_3(x) + self.flow_res3_3(x)),1)
        x = torch.cat((x, self.flow_conv3_4(x) + self.flow_res3_4(x)),1)
        ...
        if self.pred_occ and self.cat_occ:
            x = torch.cat((x, F.interpolate(occ3, scale_factor=2, mode='bilinear')), 1)
        x = torch.cat((x, self.flow_conv2_0(x) + self.flow_res2_0(x)),1)
        x = torch.cat((x, self.flow_conv2_1(x) + self.flow_res2_1(x)),1)
        x = torch.cat((x, self.flow_conv2_2(x) + self.flow_res2_2(x)),1)
        x = torch.cat((x, self.flow_conv2_3(x) + self.flow_res2_3(x)),1)
        x = torch.cat((x, self.flow_conv2_4(x) + self.flow_res2_4(x)),1)
