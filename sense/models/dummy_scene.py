...
from sense.models.psmnext import PSMNextEncoder
...
class SceneNeXt(nn.Module):
    def __init__(self, args, num_channels=[32, 32, 64, 128, 128]
        ):
        super(SceneNeXt, self).__init__()
        self.encoder = PSMNextEncoder(args.bn_type, args.kernel_size, True)
        self.flow_decoder = PWCFlowDecoder(encoder_planes=num_channels,
                                    md=args.corr_radius,
                                    refinement_module=args.flow_refinement,
                                    bn_type=args.bn_type,
