...
def make_dec_layer(in_planes, out_planes, kernel_size=3, stride=1, padding=1, dilation=1, bias=True, bn_type='syncbn'):
    return nn.Sequential(
        ...
        make_bn_layer(bn_type, in_planes),
        conv(in_planes, 2 * in_planes, kernel_size=1, stride=1, padding=0, dilation=1, bias=True),
        nn.GELU(),
        conv(2 * in_planes, out_planes, kernel_size=1, stride=1, padding=0, dilation=1, bias=True),
    ), nn.Sequential(make_bn_layer(bn_type, in_planes), nn.Conv2d(in_planes, out_planes, kernel_size=1, stride=1, padding=0, dilation=1, bias=True), nn.GELU())